...
@bot.message_handler(content_types=['sticker', 'pinned_mesage', 'photo', 'audio', 'video'])
def warning(message):
    bot.send_message(message.chat.id, f'Я тебя не понимаю. Введите: /help.')

@bot.message_handler(commands=['start'])
def start(message):
    bot.send_message(message.chat.id, f'Hello, *{message.from_user.first_name}!*\nВ любой непонятной ситуации введи\nкоманду: /help\nДа! кнопки скоро появятся ;)\nЧтобы вызвать главное меню введите: /main')
...
def main(message):
    if message.text == '/main':
        bot.send_message(message.chat.id, f'Выберите пункт меню, введя соответсвующую команду: \n/1 - Показать все записи.\n/2 - Найти номер по фамилии.\n/3 - Найти номер по имени.\n/4 - Поиск по номеру.\n/5 - Добавить запись.\n/6 - Изменить запись.\n/7 - Удалить запись.')
        ...
        bot.send_message(message.chat.id, f'Введите имя: ')
        bot.register_next_step_hamdler(message, get_name)

    elif message.text == '/6':
        lg.logging.info('The user has selected item number 6')
        bot.send_message(message.chat.id, f'Какой контакт хотите изменить?\nУкжите по:\n/61 - Фамилии\n/62 - Имени\n/63 - Номеру телефона')
        bot.register_next_step_handler(message, edit_entry)

    elif message.text == '/7':
        lg.logging.info('The user has selected item number 7')
        bot.send_message(message.chat.id, f'Выберите контакт, который хотите удалить.\nВыберите по:\n/71 - Фамилии\n/72 - Имени\n/73 - Номеру тел')
...
def get_email(message):
    ...
    bot.send_message(message.chat.id, f'Контакт добавлен!')
